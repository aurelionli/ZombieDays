...
namespace FPS
{
    public class ShotGunController : BaseWeapon
    {
      ...
      //  private SkinnedMeshRenderer silencer;//消音器
        ...
        {
            base.Awake();
            weaponType = WeaponType.Shot;
            auto = false;
            bwData = IC.GetData<Player_SO>().SData;
        }
        ...
        {
            InitListenEvent();
            resuableData.weaponType = WeaponType.Shot;
        ...
        protected override void InitFindGameObject()
        {
            ...
            Transform temp = GameTools.GetSingleComponentInChild<Transform>(gameObject, "Bullet Spawnpoints");
            for (int i = 0; i< temp.childCount;i++)
            {
                bulletShootPointGroup.Add(temp.GetChild(i));
            }
